...
# def Remind():
#     import Tkinter, tkMesspipageBox
#     root = Tkinter.Tk()
...
def CrossValidation(X, y, model, splits=10, metric=mean_absolute_error):
    ...
    for train_index, test_index in kf.split(X):
        ...
        print('' + ':' + str(currentQuota))
    print("----------------------------------------")
    print("[mean] " + ': ' + str(quota))
    print("[max] " + ': ' + str(max_quota))
    print("[min] " +  ': ' + str(min_quota))
    return
...
class MultinomialLogisticRegression:
    ...
    def calculate_loss(self, X, y):
        ...
        # uncorrelatedRegular = np.dot(self.W,self.W.T)
        vectors_norm = np.linalg.norm(self.W, axis=0)**2  # 1
        # uncorrelatedRegular = [sum([vectors_norm[j] for j in range(len(vectors_norm)) if j != i]) for i in range(len(vectors_norm))] * self.W  # 2
        (np.dot(vectors_norm.reshape(-1, 1), vectors_norm.reshape(1, -1)).sum()-(vectors_norm**2).sum())/2
        # loss += 0.5 * self.eta * (sum(map(lambda theta: (theta.reshape(-1, 1) * self.W).sum(), self.W.T)) - (self.W * self.W).sum())
        loss += 0.5 * self.eta * (np.dot(vectors_norm.reshape(-1, 1), vectors_norm.reshape(1, -1)).sum()-(vectors_norm**2).sum())/2
        softmax[np.arange(len(softmax)), y_index] -= 1
        # dW = X.T.dot(softmax) / sample_size + self.reg_strength * self.W + self.eta * (np.linalg.norm(self.W)**2 * self.W - self.W**3)
        dW = X.T.dot(softmax) / sample_size + self.reg_strength * self.W + self.eta * self.W * [sum([vectors_norm[j] for j in range(len(vectors_norm)) if j != i]) for i in range(len(vectors_norm))]
        return loss, dW
...
def main():
    X_train, y_train = ReadFile(r"/Users/ch_cmpter/Downloads/UMLR/image.csv")
    ...
    CrossValidation(X_train, y_train, sfm, 10, accuracy_score)
